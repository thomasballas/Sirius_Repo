...
//			Scanner scans = new Scanner(new File(args[1]));
			File payload = new File(args[1]);
			FileInputStream fizzy = new FileInputStream(payload);
			int fileLength = fizzy.available(); 
			msg = new byte[fileLength];
			fizzy.read(msg, 0, fileLength);
			String nelson = "nnnnnn";
			byte[] delimeter = nelson.getBytes();
			byte[] fullMessage = new byte[msg.length + delimeter.length];
			System.arraycopy(msg,0,fullMessage,0,msg.length);
			System.arraycopy(delimeter,0,fullMessage,msg.length,delimeter.length);
			//fileLength = fullMessage.length;
			...
			ImageIO.write(embed(fullMessage, img), "png", outputfile);
		} catch (IOException e) {
	...
	/*
	 * embed() takes in a byte array "msg" and a BufferedImage "img".  It assesses the input image for x and y bounds.  
	 * Then, it proceeds to create an eight place array to hold the individual bits of a single byte in msg.
	 * The algorithm isolates each bit in the byte and resets the array place to 1 if the value is non-zero.
	 ...
	 * */
	static BufferedImage embed(byte[] msg, BufferedImage img)
	{
		...
		while (y < (scaleY-1))
		{
			...
			while (x < (scaleX-1))
			{
				...
				if (bitCounter%8 == 0){
					...
					//FILL THE ARRAY WITH NEW LSB VALUES -> BIT VALUE OF MESSAGE TEXT BEGINNING WITH 
					//THE MSB AT CHARS[0] AND ENDING WITH THE LSB AT CHARS[7]
					for (int i = 0; i<8; i++)
					{
						//ISOLATE THE APPROPRIATE BIT
						chars[i] = (msg[curChar] & (1<<(7-i)));
						//IF THE RESULTING VALUE IS NOT 0, REASSIGN TO 1
						if (chars[i] != 0) chars[i] = 1;  
					}
					//AFTER DISSECTING A BYTE, INCREMENT THE COUNTER SO WE CAN PROCEED THROUGH THE STRING
					curChar++;
				}
